...
class DoctorController {
    ...
    async updateDoctor(req, res) {
        try {
            ...
            const doctor = await doctorService_1.doctorService.updateDoctor(id, updateData);
            res.status(200).json(doctor);
        }
        catch (error) {
            res.status(400).json({ error: error.message });
    ...
    async updateDoctorSchedule(req, res) {
        try {
            ...
            switch (doctorName?.toLowerCase()) {
                case 'yosef':
                    schedule = (0, scheduleHelpers_1.generateDrYosefSchedule)();
                    break;
                case 'ali_abid':
                case 'ali abid':
                    schedule = (0, scheduleHelpers_1.generateDrAliAbidSchedule)();
                    break;
                case 'adel_shalata':
                case 'adel shalata':
                    schedule = (0, scheduleHelpers_1.generateDrAdelShalataSchedule)();
                    break;
                default:
                    res.status(400).json({ error: 'Invalid doctor name. Use: yosef, ali_abid, or adel_shalata' });
                    return;
            }
            const updatedDoctor = await doctorService_1.doctorService.updateDoctor(id, { schedule });
        ...
        try {
            ...
            if (daySchedules.length === 0) {
                res.status(200).json({
                    ...
                    dayOfWeek: day,
                    dayName: (0, scheduleHelpers_1.getDayName)(day),
                    slots: [],
     ...
     */
    async getDoctorWeeklySchedule(req, res) {
        try {
            const { id } = req.params;
            const doctor = await doctorService_1.doctorService.getDoctorById(id);
            if (!doctor) {
